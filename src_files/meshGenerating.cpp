...
void createUnitCube(Mesh& mesh) {
    ...
    objectToTextureIndex.push_back(0);
...
void createUVSphere(Mesh& mesh, int slices, int stacks) {
    ...
    // --- Vertex Generation ---
    // (Logic kept exactly as is to preserve Normal/UV placeholders and Seam logic)
    for (int i = 0; i <= stacks; ++i) {
        float lat = static_cast<float>(i) / static_cast<float>(stacks) * PI;
        for (int j = 0; j <= slices; ++j) {
            float lon = static_cast<float>(j) / static_cast<float>(slices) * 2.0f * PI;
            float x = std::cos(lon) * std::sin(lat);
            float y = std::cos(lat);
            float z = std::sin(lon) * std::sin(lat);

            // Placeholder attributes preserved as requested
            mesh.vertices.push_back({{x,y,z}, {1,1,1}, {0,0}, {0,0,0}, 0});
        }
    }
...
void createIcosahedron(Mesh& mesh) {
    ...
    std::vector<Vec3> verts = { {-1,t,0},{1,t,0},{-1,-t,0},{1,-t,0},{0,-1,t},{0,1,t},{0,-1,-t},{0,1,-t},{t,0,-1},{t,0,1},{-t,0,-1},{-t,0,1} };
    for(auto& v : verts) {
        Vec3 n = norm(v);
        mesh.vertices.push_back({n, {1,1,1}, {0,0}, {0,0,0}, 0});
    }
    mesh.indices = { {0,11,5},{0,5,1},{0,1,7},{0,7,10},{0,10,11},{1,5,9},{5,11,4},{11,10,2},{10,7,6},{7,1,8},{3,9,4},{3,4,2},{3,2,6},{3,6,8},{3,8,9},{4,9,5},{2,4,11},{6,2,10},{8,6,7},{9,8,1} };
}
