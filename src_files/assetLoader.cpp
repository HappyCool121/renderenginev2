...
bool loadGLTF(const std::string &filename, Mesh &mesh) {
  ...
  // Mapping: glTF Texture Index (0, 1, 2...) -> Engine Texture Index (Start at
  // 1, 2...)
  ...
  for (size_t i = 0; i < model.textures.size(); ++i) {
    ...
    if (sourceImageIdx >= 0 && sourceImageIdx < model.images.size()) {
      ...
      std::cout << "Loaded GLTF Tex [" << i << "] -> Engine Tex ["
                << gltfToEngineTexIndex[i] << "] (" << newTex.width << "x"
                << newTex.height << ")" << std::endl;
    }
  }
  ...
  for (const auto &gltfMesh : model.meshes) {
    for (const auto &primitive : gltfMesh.primitives) {
      ...
      if (primitive.indices >= 0) {
        ...
        // 2. Extract raw indices into a temporary vector to handle types (Short
        // vs Int) This removes the need to duplicate logic for every topology
        // type
        ...
        if (idxAccessor.componentType ==
          ...
          const unsigned short *buf =
              reinterpret_cast<const unsigned short *>(dataPtr);
          for (size_t i = 0; i < idxAccessor.count; ++i) {
            rawIndices.push_back(buf[i]);
          }
        } else if (idxAccessor.componentType ==
                   TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT) {
          const unsigned int *buf =
              reinterpret_cast<const unsigned int *>(dataPtr);
          for (size_t i = 0; i < idxAccessor.count; ++i) {
            rawIndices.push_back(buf[i]);
          }
        } else if (idxAccessor.componentType ==
        ...
        if (mode == TINYGLTF_MODE_TRIANGLES) {
          ...
          for (size_t i = 0; i < rawIndices.size() - 2; ++i) {
            ...
            mesh.indices.push_back({a + vertexStartOffset,
                                    b + vertexStartOffset,
                                    c + vertexStartOffset});
          }
  ...
  std::cout << " current object ID (custom import after) " << currentObjectID
            << std::endl;
