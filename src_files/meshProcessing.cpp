...
Vertex interpolateVertex(const Vertex &v1, const Vertex &v2, float t) {
  Vertex m;
  m.pos = {v1.pos.x + (v2.pos.x - v1.pos.x) * t,
           v1.pos.y + (v2.pos.y - v1.pos.y) * t,
           v1.pos.z + (v2.pos.z - v1.pos.z) * t};

  // Lerp other attributes
  m.uv = {v1.uv.x + (v2.uv.x - v1.uv.x) * t, v1.uv.y + (v2.uv.y - v1.uv.y) * t};
  m.color = {v1.color.x + (v2.color.x - v1.color.x) * t,
             v1.color.y + (v2.color.y - v1.color.y) * t,
             v1.color.z + (v2.color.z - v1.color.z) * t};
  m.objectID = v1.objectID;

  // Normal interpolation
  Vec3 tmpN = {v1.normal.x + (v2.normal.x - v1.normal.x) * t,
               v1.normal.y + (v2.normal.y - v1.normal.y) * t,
               v1.normal.z + (v2.normal.z - v1.normal.z) * t};
...
void doSubdivide(Mesh &mesh, bool spherize, SubdivMode mode) {
  ...
  std::vector<Vertex> newVertices =
      mesh.vertices; // Starts with copy of old vertices
  ...
  for (int faceIdx = 0; faceIdx < mesh.indices.size(); ++faceIdx) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      auto addUnique = [&](int target, int val) {
        for (int n : vertexNeighbors[target])
          if (n == val)
            return;
      ...
      if (mode == Loop) {
        ...
        // CC needs connected Vertices to find edges later, but simple neighbor
        // list is fine
        addUnique(a, b);
        addUnique(b, a);
      }
  ...
  if (mode == catmullClark) {
    ...
    for (const auto &t : mesh.indices) {
      ...
      // Average Attributes
      fPoint.normal = safeNorm({(v0.normal.x + v1.normal.x + v2.normal.x),
                                (v0.normal.y + v1.normal.y + v2.normal.y),
                                (v0.normal.z + v1.normal.z + v2.normal.z)});
      fPoint.uv = {(v0.uv.x + v1.uv.x + v2.uv.x) / 3.0f,
                   (v0.uv.y + v1.uv.y + v2.uv.y) / 3.0f};
      fPoint.color = {(v0.color.x + v1.color.x + v2.color.x) / 3.0f,
                      (v0.color.y + v1.color.y + v2.color.y) / 3.0f,
                      (v0.color.z + v1.color.z + v2.color.z) / 3.0f};
      fPoint.objectID = v0.objectID;

      if (spherize)
        fPoint.pos = safeNorm(fPoint.pos);
    ...
    for (auto &pair : edgeMap) {
      ...
      if (e.o2 == -1) {
        ...
        // Interpolate other attributes linearly between original verts for
        // simplicity
      ...
      if (spherize)
        ePoint.pos = safeNorm(ePoint.pos);
    ...
    for (int i = 0; i < mesh.vertices.size(); ++i) {
      ...
      // Note: CC definition uses pure midpoints of edges, not the new
      // EdgePoints calculated in step B
      Vec3 R_avg = {0, 0, 0};
      ...
      newV.pos.x =
          (F_avg.x * weightF) + (R_avg.x * weightR) + (oldV.pos.x * weightOld);
      newV.pos.y =
          (F_avg.y * weightF) + (R_avg.y * weightR) + (oldV.pos.y * weightOld);
      newV.pos.z =
          (F_avg.z * weightF) + (R_avg.z * weightR) + (oldV.pos.z * weightOld);

      if (spherize)
        newV.pos = safeNorm(newV.pos);
    }
  ...
  // =========================================================
    ...
    for (auto &pair : edgeMap) {
      ...
      if (mode == Loop && e.o2 != -1) {
        ...
        m.pos.x =
            (v1.pos.x + v2.pos.x) * 0.375f + (o1.pos.x + o2.pos.x) * 0.125f;
        m.pos.y =
            (v1.pos.y + v2.pos.y) * 0.375f + (o1.pos.y + o2.pos.y) * 0.125f;
        m.pos.z =
            (v1.pos.z + v2.pos.z) * 0.375f + (o1.pos.z + o2.pos.z) * 0.125f;
      }

      if (spherize)
        m.pos = safeNorm(m.pos);
            ...
            float beta =
                (n == 3)
                    ...
                    : (1.0f / n) *
                          (0.625f -
                           std::pow(0.375f +
                                        0.25f * std::cos(2.0f * 3.14159f / n),
                                    2));
            float selfWeight = 1.0f - (n * beta);
            newV.pos = {oldV.pos.x * selfWeight, oldV.pos.y * selfWeight,
                        oldV.pos.z * selfWeight};
